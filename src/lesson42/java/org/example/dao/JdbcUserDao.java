...
public class JdbcUserDao implements UserDao {
    ...
    public JdbcUserDao(DataSource dataSource) {
        ...
        this.userRowMapper = new UserRowMapper(addressRowMapper);
    ...
    public User findById(int id) {
        String userSql = "SELECT id, name, email FROM users WHERE id = ?";
        String addressSql = "SELECT id, user_id, street, city, postal_code FROM addresses WHERE user_id = ?";

        User user = template.queryForObject(userSql, userRowMapper, id);
        List<Address> addresses = template.query(addressSql, addressRowMapper, id);
